...
Section SEMANTICS.
  ...
  Next Obligation.
    intros ? [? [? ?]] [? [? ?]] H1 H2 H3.
    exploit HL. apply H1. apply H2. apply H3.
    now cbn.
  Qed.
End SEMANTICS.

Section SEMANTICS_DETERM.
  Context {liA liB} (L : semantics liA liB) (HL : determinate L).

  Lemma star_step_determ se s tr s1 s2 t r t' r':
    Star (L se) s tr s1 ->
    lts_trace (L se) false s1 t r ->
    Star (L se) s tr s2 ->
    lts_trace (L se) false s2 t' r' ->
    s1 = s2.
  Proof.
    intros st1 lts1 st2 lts2.
    exploit (star_determinacy (HL se)); [ exact st1 | exact st2 | ].
    intros [ | ].
    + inv H; inv lts1; auto.
  ...
  Lemma trace_determ se b s t t' r r' :
    list_coh liA t t' ->
    lts_trace (L se) b s t r ->
    lts_trace (L se) b s t' r' ->
    t = t' /\ r = r'.
  Proof.
    intros coh h h'.
    revert t' coh h'.
    induction h; intros t' coh lts; inversion lts; subst.
    + assert (s' = s'0) as <- by eauto using star_step_determ.
      apply IHh; auto.
    ...
    + specialize (IHh t0).
      ...
      inv coh. inv H7. exploit H5; auto. intros <-.
      determ_solve (sd_after_external_determ (HL se)).
      split; f_equal; apply IHh; auto.
  Qed.

  Lemma HL': sem_coherence L.
  Proof.
  ...
  Definition compcerto_lmap se: !liA --o liB := compcerto_lmap' L HL' se.
End SEMANTICS_DETERM.
...
Definition coh_refinement {liA liB1 liB2} (ccB: callconv liB1 liB2)
           (Σ1: Genv.symtbl -> !liA --o liB1)
           (Σ2: Genv.symtbl -> !liA --o liB2) sk :=
  forall se1 se2 wB,
...
Section GC_CC.
  ...
  Hypothesis spec_vq:
    forall se t q r, has (Σ se) (t, (q, r)) -> valid_query (L se) q = true.
  Let sk := skel L.
  Let vq := (fun se => valid_query (L se)).
  Lemma sem_unsem_gc cc:
    coh_refinement cc Σ (compcerto_lmap' L HL) sk <->
    forward_simulation cc_id cc (UnSem.semantics sk Σ vq) L.
  Proof.
  (* split. *)
  (* - set (LΣ := UnSem.semantics _ _ _). *)
  (*   intros MATCH. constructor. *)
