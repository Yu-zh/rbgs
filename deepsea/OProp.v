...
Notation "x #{ f  }" := (omap1 (fun p y => p (f y)) x%oprop1)
...
Notation "x #{ f  } #{ g  }" := (omap2 (fun p y z => p (f y) (g z)) x%oprop2)
         (at level 25, left associativity) : oprop2_scope.
Notation "x #{ f  } #{ g  }" := (omap21 (fun p y => p (f y) (g y)) x%oprop2)
         (at level 25, left associativity) : oprop1_scope.
...
Notation "x #{ f  } #{ g  } #{ h  }" := (omap3 (fun p y z w => p (f y) (g z) (h w)) x%oprop3)
